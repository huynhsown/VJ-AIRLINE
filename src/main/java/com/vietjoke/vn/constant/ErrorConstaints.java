package com.vietjoke.vn.constant;

public final class ErrorConstaints {
    public static String SESSION_EXPIRED = "SESSION_EXPIRED";
    public static String MISSING_BOOKING_STEP = "MISSING_BOOKING_STEP";
    public static String ERROR_BOOKING_STEP = "ERROR_BOOKING_STEP";
    private static final String FLIGHT_SELECTION_MISSING = "Flight selection step is missing";
    private static final String PASSENGER_TYPE_REQUIRED = "Passenger type is required";
}
