...
public class MasterSeeder implements CommandLineRunner {
    ...
    private void seedCountries() throws IOException {
        InputStream inputStream = new ClassPathResource("/data/only_countries.json").getInputStream();
        List<CountryEntity> countryEntities = objectMapper.readValue(inputStream, new TypeReference<List<CountryEntity>>() {});
        log.info("Countries: {}", countryEntities);
        for(CountryEntity countryEntity : countryEntities) {
            if(!countryRepository.existsByCountryCode(countryEntity.getCountryCode())) {
    ...
    private void seedPromos(){
        try {
            if(promoRepository.count() != 0) return;
            InputStream inputStream = new ClassPathResource("/data/promo.json").getInputStream();
            List<PromoCodeDTO> promoCodeDTOS = List.of(objectMapper.readValue(inputStream, PromoCodeDTO[].class));
            List<PromoCodeEntity> promoCodeEntities = new ArrayList<>();
            for(PromoCodeDTO promoCodeDTO : promoCodeDTOS){
                PromoCodeEntity promoCodeEntity = promoConverter.toPromoEntity(promoCodeDTO);
